...
protected:
    ...
    void flush_() override { sentry_flush(1000); }
...
private:
    ...
    void CaptureEvent(const spdlog::details::log_msg &msg) const
    {
        sentry_value_t event = sentry_value_new_message_event(
            SentryLevelForSpdLogLevel(msg.level), LoggerName(msg).c_str(), LogMessage(msg).c_str());
    ...
    void CaptureBreadcrumb(const spdlog::details::log_msg &msg) const
    {
        ...
        sentry_value_set_by_key(
            crumb,
            "level",
            sentry_value_new_string(SentryLevelNameForSpdLogLevel(msg.level).c_str()));
